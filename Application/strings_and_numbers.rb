def decode(string) # string is a string aight
  ...
  0.upto(string.length) do |i|
   until string[i] != decode_hash.key(decode_hash[string[i]])
     if i > string.length then break else i += 1
     end
   end
  ...
  # take first 10 most repeating characters to decode

string.split('').each { |i| if ( decode_hash.key?(string[i]) ) then string[i] = decode_hash.keys.reverse.index(decode_hash.key(decode_hash[string[i]])).to_s end }
